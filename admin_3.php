...
$result = mysql_query('SELECT * FROM subpage WHERE id = "4"',$mysql)
...
if(isset($_POST['save']))
{
	$inquiry = "UPDATE subpage SET contents = '".$site_contents."',pagename = '".$site_pagename."' WHERE id = '4'";
	...
	else
	{
		...
		$result = mysql_query('SELECT * FROM subpage WHERE id = "4"');
		$record = mysql_fetch_array($result, MYSQL_ASSOC);
		$contents = $record['contents'];
		$pagename = $record['pagename'];
	}
...
<form action="admin.php?admin=statut" method="post">
