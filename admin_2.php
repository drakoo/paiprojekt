...
$result = mysql_query('SELECT * FROM subpage WHERE id = "3"',$mysql)
...
if(isset($_POST['save']))
{
	$inquiry = "UPDATE subpage SET contents = '".$site_contents."',pagename = '".$site_pagename."' WHERE id = '3'";
	...
	else
	{
		...
		$result = mysql_query('SELECT * FROM subpage WHERE id = "3"');
		$record = mysql_fetch_array($result, MYSQL_ASSOC);
		$contents = $record['contents'];
		$pagename = $record['pagename'];
	}
...
<form action="admin.php?admin=sprawozdania" method="post">
